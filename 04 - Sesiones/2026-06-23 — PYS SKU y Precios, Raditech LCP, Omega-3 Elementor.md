...
- [~] **PYS:** re-medir PageSpeed cuando termine QUIC.cloud. ESTADO 2026-06-23 (rev): optimización **al 47%**, `Images Pulled: -` / `Last Request: -` → NO había completado ni un ciclo (el auto-cron de WP probablemente no disparaba). Ajustes correctos (Auto Request Cron ON, WebP ON, Optimize Original ON, backups conservados). **Enviado 1 "Send Optimization Request" manual** (límite inicial QUIC = 1 grupo/request; sube tras el primer Pull exitoso). Falta: dejar que el cron procese los ~64 grupos restantes (o empujar manualmente), luego **Pull Images** + purgar + medir. PSI API bloqueada por cuota (sin key) — medir en pagespeed.web.dev.
